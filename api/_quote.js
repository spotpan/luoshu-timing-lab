...
export function eastmoneySecid(symbol){
  const s=String(symbol||'').trim().toUpperCase();
...
export function calcMA3060(points){
  ...
  if(prevDiff<=0 && diff>0){status='golden_cross';label='30/60均线金叉';boost=6;message='30日均价刚刚上穿60日均价，中期趋势转强观察。';}
  else if(diff>0){status='above';label='均线上方保持';boost=3;message='30日均价保持在60日均价上方，中期趋势仍有支撑。';}
  else if(rel<0.006){status='near';label='均线临界观察';boost=1;message='30日均价接近60日均价，处在趋势临界区，需等待确认。';}
  let observeScore=50;
  ...
  return {status,label,boost,message,price:+price.toFixed(4),prevClose:+prevClose.toFixed(4),date:rows[rows.length-1]?.date||'',ma30:+ma30.toFixed(4),ma60:+ma60.toFixed(4),prevMa30:+prevMa30.toFixed(4),prevMa60:+prevMa60.toFixed(4),ma30Slope:+ma30Slope.toFixed(3),ma60Slope:+ma60Slope.toFixed(3),bias30:+bias30.toFixed(3),bias60:+bias60.toFixed(3),observeScore,riskLabel};
}
export async function fetchYahoo(symbol){
  ...
  if(!Array.isArray(closes)||!Array.isArray(times)) throw new Error('Yahoo缺少日线');
  const points=closes.map((c,i)=>({date:new Date(times[i]*1000).toISOString().slice(0,10),close:Number(c)})).filter(p=>Number.isFinite(p.close));
  if(points.length<61) throw new Error(`Yahoo日线点不足：${points.length}`);
  return {points,source:`Yahoo(${ys})`};
}
export async function fetchStooq(symbol){
  ...
  const pts=lines.slice(1).map(line=>{const p=line.split(',');return {date:p[dateIdx],close:parseFloat(p[closeIdx])};}).filter(x=>Number.isFinite(x.close)).sort((a,b)=>a.date.localeCompare(b.date));
  if(pts.length<61) throw new Error(`Stooq可解析点不足：${pts.length}`);
  return {points:pts,source:`Stooq(${ss})`};
}
export async function fetchEastmoney(symbol){
  ...
  const pts=arr.map(line=>{const p=String(line).split(',');return {date:p[0],close:parseFloat(p[2])};}).filter(x=>Number.isFinite(x.close));
...
export async function fetchMarketMA(symbol){
  const attempts=[];
  const sources=[['Yahoo Chart',()=>fetchYahoo(symbol)],['东方财富日线',()=>fetchEastmoney(symbol)],['Stooq CSV',()=>fetchStooq(symbol)]];
  for(const [name,fn] of sources){
    try{const out=await fn();const sig=calcMA3060(out.points);return {ok:true,symbol,source:out.source,attempts:[...attempts,{name,ok:true,msg:out.source}],...sig};}
    catch(e){attempts.push({name,ok:false,msg:e.message||String(e)});}
  }
  return {ok:false,symbol,source:'Vercel K线代理',status:'unknown',label:'均线信号待确认',boost:0,price:null,ma30:null,ma60:null,message:'Vercel K线代理未能获取到足够历史日线。',attempts,error:attempts.map(a=>`${a.name}: ${a.msg}`).join('；')};
}
